...
% zero numbers in both sides
zxp = floor(pattern_size(2)/2);
zyp = floor(pattern_size(1)/2);

% extend input image and pattern with zeros
input_image_no_DC_extend = zeros(2*zyp + input_size(1),2*zxp + input_size(2));
input_image_no_DC_extend(zyp+1:input_size(1)+zyp , zxp+1:input_size(2)+zxp) = input_image_no_DC;

% Main loops for sliding the pattern over the input image                             
h   = waitbar(0 , 'Direct evaluation of the correlation');
pndcn = sqrt(sum(pattern_no_DC.^2,"all")); % Pattern no DC norm (to normalize output)
for row_ind = 1 : input_size(1)                                                        %%%%%%%
    waitbar( row_ind / input_size(1) )                                                 %%%%%%%
      for col_ind = 1 : input_size(2)
          % Cut effective matrix
          effectiveMatrix = input_image_no_DC_extend(row_ind:row_ind+pattern_size(1)-1,col_ind:col_ind+pattern_size(2)-1);
          % Calculate correlation matrix
          correlationMatrix = effectiveMatrix.*pattern_no_DC;
          % add all correlationMatrix elements and put in a element output
          normalized_correlation(row_ind,col_ind) = sum(correlationMatrix,"all");
          emn = sqrt(sum(effectiveMatrix.^2,"all")); % Effective matrix norm (to normalize output)
          normalized_correlation(row_ind,col_ind) = normalized_correlation(row_ind,col_ind)/(pndcn*emn);
      end
end
close(h)                                                                               %%%%%%%
                                                                                       %%%%%%%
